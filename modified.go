...
func lazyCaterer(sequenceNum int) int {
	return (int(math.Pow(float64(sequenceNum), float64(2))) + int (math.Pow(float64(sequenceNum), float64(3))) + sequenceNum + 2 ) / 2
}
	...
	for {
		...
		if strings.Compare(message, "ACK\n") == 0 {
			...
			if inCongestionAvoidanceMode && !maxCapacityReached {
				...
				fmt.Println("Generating next number in Modified's sequence")
			...
			if inCongestionAvoidanceMode {
				...
				fmt.Println("Throughput = ", f(float64(totalPacketsSent))/float64(time.Since(startTime)))
				return
